...
function Courses() {
  ...
  /**
   * Generates and downloads a CSV file of the current courses list
   * using standard Blob approach and escaping logic.
   */
  const handleDownloadCSV = () => {
    ...
    const csvRows = [
      ...
      // 2. Data rows with escaping logic
      ...courses.map(course => 
        headers.map(header => {
          const val = course[header] === null || course[header] === undefined 
            ? "" 
            : String(course[header]);
          
          // Escape double quotes by doubling them and wrap entire value in double quotes
    ...
    link.setAttribute('download', `courses_catalog_${new Date().toISOString().split('T')[0]}.csv`);
